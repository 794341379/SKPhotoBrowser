...
open class SKPhotoBrowser: UIViewController {
    ...
    public convenience init(photos: [SKPhotoProtocol], initialPageIndex: Int, animatedFromView: UIView? = nil) {
        self.init(nibName: nil, bundle: nil)
        self.photos = photos
        //self.photos.forEach { $0.checkCache() }
        ...
        animator.senderViewForAnimation = animatedFromView ?? photos[currentPageIndex] as? UIView
    }
